...
import { NgClass, NgStyle, NgIf, NgFor, NgTemplateOutlet, DecimalPipe } from '@angular/common';
...
@Component({
    selector: 'tjrj-datagrid',
    templateUrl: 'data-grid.component.html',
    styleUrls: ['data-grid.component.scss'],
    standalone: true,
    imports: [NgClass, NgStyle, NgIf, FormsModule, NgFor, PopoverModule, ColumnFilterComponent, NgTemplateOutlet, SpinnerComponent, BsDropdownModule, PaginationModule, DecimalPipe]
})
export class DataGridComponent implements OnInit, DoCheck, DataGridConfig {
  ...
  @Input() emptyResultsMessage?: string = 'Nenhum registro encontrado.';
  // @Input() infoMessage?: string = 'Showing records from {recordsFrom} to {recordsTo} of {totalRecords} records found.';
  @Input() infoMessage?: string = 'Exibindo registro {recordsFrom} até {recordsTo} de {totalRecords} registros encontrados.';
  ...
  @Input() exportedFileName?: string = 'Exportar';
  ...
  @Input() firstText?: string = 'Primeiro';
  @Input() previousText?: string = 'Anterior';
  @Input() nextText?: string = 'Próximo';
  @Input() lastText?: string = 'Último';
  ...
  @Input() filterPlaceholder?: string = 'Filtro...';
  @Input() filterPlacement?: string = 'bottom';
  @Input() showItemsPerPageSelect?: boolean = false;
  ...
  @Output() OnItemsPerPageChanged = new EventEmitter<any>();
  ...
  OnChangeTableRowColor(row: any, rowIndex: number) {    
    if(this.allowRowClick) {
      if (this.rowClicked === rowIndex) this.rowClicked = -1;
      else this.rowClicked = rowIndex;
  ...
  OnItemPerPageChange() {
    if (this.mode == EnumDataGridMode.OnServer) {
      this.OnItemsPerPageChanged.emit({itemsPerPage: this.itemsPerPage});
    } else {
      this.initializeRendering();
    }
  }
